...
def execute_test_de_rangos(data1, data2):
...
def main(instance1_name, instance2_name, ae_quantity):
    with open(f'output/output_test_de_rangos_{instance1_name}_{instance2_name}.txt', 'w') as f:
        f.write(f"Test de Rangos for {instance1_name} and {instance2_name} Instances\n\n")

    for i in range(ae_quantity):
        print(f"[INFO] Test for AE {i+1}")
        with open(f'output/output_test_de_rangos_{instance1_name}_{instance2_name}.txt', 'a') as f:
            f.write(f"AE {i+1}\n")
        ...
        print("# Fitness")
        output_text = execute_test_de_rangos(fitness_1, fitness_2)
        with open(f'output/output_test_de_rangos_{instance1_name}_{instance2_name}.txt', 'a') as f:
            f.write('# Fitness\n')
            f.write(output_text)
        
        print("# Variety")
        output_text = execute_test_de_rangos(variety_1, variety_2)
        with open(f'output/output_test_de_rangos_{instance1_name}_{instance2_name}.txt', 'a') as f:
            f.write('# Variety\n')
            f.write(output_text)
            f.write('\n')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Diet Problem - Test de Rangos')
