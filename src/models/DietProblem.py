...
class DietProblem(IntegerProblem):
    ...
    def __init__(self, number_of_meals: int, number_of_days: int, max_portions: int, food_ids: int, food_objects: list, config: Config):
        ...
        self.upper_bound = [len(food_ids) for _ in range(number_of_meals * number_of_days * max_portions)]
    ...
    def evaluate(self, solution: IntegerSolution) -> IntegerSolution:
        ...
        for day in range(self.number_of_days):
            ...
            for food in range(self.number_of_meals):
                for portion in range (1, self.max_portions):
                    ...
                    if (c == 0):
                        ...
                        if meal is None:
                            raise ValueError(f"No meal found with id {c - 1}")
    ...
    def create_solution(self) -> IntegerSolution:
        ...
        for i in range(self.number_of_days * self.number_of_meals * self.max_portions):
            new_solution.variables[i] = np.random.choice([0] + self.food_ids)
